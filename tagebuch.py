...
def tagebuch_schreiben():
    ...
    datei_pfad = f"tagebuch/{jahr}/{woche}_{tag}.txt"

    if not os.path.exists(f"tagebuch/{jahr}"):
        os.mkdir(f"tagebuch/{jahr}")
...
def tagebuch_anzeigen():
    ...
    count = 0
    for verzeichnis in os.listdir(f"tagebuch/{jahr}"):
        if os.path.isfile(f"tagebuch/{jahr}/{verzeichnis}"):
            count += 1
...
def tagebuch_eintraege_anzeigen():
    ...
    datei_pfad = f"tagebuch/{jahr}/{woche}"
